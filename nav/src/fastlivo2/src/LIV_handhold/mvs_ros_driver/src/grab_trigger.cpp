...
static void *WorkThread(void *pUser) {
  ...
  while (!exit_flag && ros::ok()) {
    ...
    if (nRet == MV_OK) {
      ...
      ros::Time rcv_time;
      if(trigger_enable && pointt != nullptr && pointt->low != 0)
      {
        // 赋值共享内存中的时间戳给相机帧
...
int main(int argc, char **argv) {
  ...
  ros::NodeHandle nh;
  // image_transport::ImageTransport it(nh);  // Disabled to avoid class_loader issue
  int nRet = MV_OK;
  void *handle = NULL;
  ...
  pub = nh.advertise<sensor_msgs::Image>(pub_topic, 1);
  ...
  if (fd != -1) {
    ...
    if (pointt == MAP_FAILED) {
      ROS_WARN("Failed to mmap timeshare file, using system time");
      pointt = nullptr;
    } else {
      ROS_INFO("Timeshare file opened successfully");
    }
    close(fd);
  } else {
    ROS_WARN("Timeshare file not found: %s, using system time", shared_file_name);
    pointt = nullptr;
  }
  ...
  if (nRet != MV_OK) {
    ROS_WARN("PixelFormat setting failed (nRet=%x), using camera default", nRet);
  }
