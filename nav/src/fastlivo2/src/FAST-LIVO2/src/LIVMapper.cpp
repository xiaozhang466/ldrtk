...
void LIVMapper::publish_frame_world(const ros::Publisher &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  if (pcd_save_en)
  {
    ...
    switch (pcd_save_type)
    {
      ...
      case 2: /** lidar frame (downsampled) **/
        if ((LidarMeasures.lio_vio_flg == LIO || LidarMeasures.lio_vio_flg == LO) && feats_down_body)
        {
          *pcl_wait_save_intensity += *feats_down_body;
          scan_wait_num++;
          cout << "save lidar frame points: " << pcl_wait_save_intensity->points.size() << endl;
        }
        pcd_save_interval = 1;
